...
@rt('/api/vacate-photo', methods=['POST'])
async def vacate_photo(request: Request):
    """
    Remove furniture and objects from a staging photo using AI inpainting

    Request body:
        {
            "image": "base64-encoded image or URL path",
            "mask": "optional base64-encoded mask (white=remove, black=keep)",
            "auto_detect": true/false,
            "save": true/false (whether to save the vacated image)
        }

    Returns:
        {
            "success": true,
            "vacated_image": "base64-encoded result",
            "saved_url": "/path/to/saved/image.jpg" (if save=true)
        }
    """
    import base64
    import io
    ...
    try:
        ...
        # Handle image input (base64 or URL path)
        if image_data.startswith('/static/'):
            # Load from file path
            image_path = image_data.lstrip('/')
        ...
        # Initialize vacator and process image
        vacator = ImageVacator(device="auto")

        # Perform vacating
        result_image, result_base64 = vacator.vacate_image(
        ...
        response_data = {
            'success': True,
            'vacated_image': f'data:image/jpeg;base64,{result_base64}'
        }

        # Save the result if requested
        if save_result:
            save_dir = 'static/images/vacated'
            os.makedirs(save_dir, exist_ok=True)

            filename = f"vacated_{uuid.uuid4().hex[:8]}.jpg"
            filepath = os.path.join(save_dir, filename)

            result_image.save(filepath, 'JPEG', quality=90, optimize=True)
            response_data['saved_url'] = f'/static/images/vacated/{filename}'
        ...
        error_msg = str(e)
        if 'simple_lama_inpainting' in error_msg or 'simple-lama-inpainting' in error_msg:
            return JSONResponse({
                'success': False,
                'error': 'Image vacating library not installed. Please run: pip3 install simple-lama-inpainting torch torchvision'
            }, status_code=500)
        return JSONResponse({'success': False, 'error': f'Import error: {error_msg}'}, status_code=500)
