...
class InpaintingTester:
    ...
    def method_5_decor8ai(
        ...
        try:
            ...
            # Generate unique image ID with timestamp to avoid caching issues
            import uuid
            image_id = f"{uuid.uuid4().hex[:12]}_{int(time.time())}"
            ...
            if not temp_path.exists():
                raise Exception(f"Failed to save image to {temp_path}")

            # Build public URL
            image_url = f"{self.base_url}/api/temp-image/{image_id}"
            ...
            if mask:
                ...
                # Save mask with unique ID (same as image but with _mask suffix)
                mask_id = f"{image_id}_mask"
                mask_path = temp_dir / f"{mask_id}.png"

                # Always save the mask
                mask.save(mask_path, format='PNG')
                print(f"Mask saved to: {mask_path}")

                # Verify file exists
                if not mask_path.exists():
                    raise Exception(f"Failed to save mask to {mask_path}")

                mask_url = f"{self.base_url}/api/temp-image/{mask_id}"
            ...
            # Add cache-busting timestamp to URLs to prevent Decor8.ai from returning cached results
            cache_bust = f"?t={int(time.time())}&r={random.randint(1000,9999)}"
            api_payload['input_image_url'] = image_url + cache_bust
            if mask_url:
                api_payload['mask_image_url'] = mask_url + cache_bust

            print(f"=== DECOR8.AI API PAYLOAD ===")
            print(f"input_image_url: {api_payload['input_image_url']}")
            print(f"mask_image_url: {api_payload.get('mask_image_url', 'NOT PROVIDED')}")
            print(f"=============================")
